...
function Icetip:NewModule(name, label, order, embedHook, slince)
	...
	modules[name] = mod;
	tinsert(modulesByIndex, mod);

	if embedHook then
		LibStub("AceHook-3.0"):Embed(mod);
	end

	sort(modulesByIndex, function(a, b) return a.order < b.order end);
...
function Icetip:GetModulesByIndex()
	return pairs(modulesByIndex)
end
...
function Icetip:CallMethodAllModules(methodName, ...)
	for _, module in self:GetModulesByIndex() do
...
function Icetip:OnInitialize()
	...
	SM:Register("font", "San Francisco", [[Interface\AddOns\Icetip\media\San Francisco Display Regular.ttf]])
